...
function r() {
  return r = Object.assign ? Object.assign.bind() : function(t) {
    for (var e = 1; e < arguments.length; e++) {
      var r2 = arguments[e];
      for (var n2 in r2)
        ({}).hasOwnProperty.call(r2, n2) && (t[n2] = r2[n2]);
    }
    return t;
...
var n = class {
  ...
  matchesUrl(e) {
    ...
    if (o2) {
      for (const t in o2.groups)
        o2.groups[t] = "string" == typeof o2.groups[t] ? decodeURIComponent(o2.groups[t]) : o2.groups[t];
  ...
  compile(t) {
    return this.parameterSegments.length ? this.template.replace(/{([^}?]+)(\??)}/g, (e, r2, n2) => {
      var i2, s2;
      if (!n2 && [null, void 0].includes(t[r2]))
        throw new Error(`Ziggy error: '${r2}' parameter is required for route '${this.name}'.`);
      if (this.wheres[r2] && !new RegExp(`^${n2 ? `(${this.wheres[r2]})?` : this.wheres[r2]}$`).test(null != (s2 = t[r2]) ? s2 : ""))
        throw new Error(`Ziggy error: '${r2}' parameter '${t[r2]}' does not match required format '${this.wheres[r2]}' for route '${this.name}'.`);
...
var i = class extends String {
  ...
  current(t, e) {
    ...
    if ([null, void 0].includes(e) || !h)
      return h;
    const a = new n(i2, u2, this.t);
    e = this.u(e, a);
    const l = r({}, s2, o2);
    if (Object.values(e).every((t2) => !t2) && !Object.values(l).some((t2) => void 0 !== t2))
      return true;
  ...
  $(t, { bindings: e, parameterSegments: n2 }) {
    return Object.entries(t).reduce((t2, [i2, s2]) => {
      if (!s2 || "object" != typeof s2 || Array.isArray(s2) || !n2.some(({ name: t3 }) => t3 === i2))
        return r({}, t2, { [i2]: s2 });
      if (!s2.hasOwnProperty(e[i2])) {
        if (!s2.hasOwnProperty("id"))
          throw new Error(`Ziggy error: object passed as '${i2}' parameter is missing route model binding key '${e[i2]}'.`);
        e[i2] = "id";
      }
...
function u(t) {
  if (!t && !globalThis.Ziggy && "undefined" == typeof Ziggy && !document.getElementById("ziggy-routes-json"))
